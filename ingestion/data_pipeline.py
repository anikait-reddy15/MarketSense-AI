...
class VectorDatabaseManager:
    def __init__(self, persist_directory: str):
        ...
        print("[INFO] Initializing HuggingFace Embeddings (Local)...")
        # all-MiniLM-L6-v2 is an extremely fast, lightweight model perfect for local text embedding
    ...
    def load_json_to_documents(self, filepath: str) -> list[Document]:
        ...
        for item in data:
            # Extract standard fields mapped from the ddgs scraper
            text_content = item.get('text', '')
            title = item.get('title', '')
            url = item.get('url', '')
            source = item.get('source', 'Unknown')
            
            # Merge title and text to provide rich context to the embedding model
    ...
    def build_vector_store(self, json_files: list[str], collection_name: str = "marketsense_trends"):
        ...
        # This automatically creates the SQLite files inside your persist_directory
...
if __name__ == "__main__":
    # Dynamically resolve the absolute paths relative to this script
    base_dir = os.path.dirname(os.path.dirname(__file__))
    raw_data_dir = os.path.join(base_dir, "data", "raw")
    vector_store_dir = os.path.join(base_dir, "data", "vector_store")
    
    # Target the files generated by trend_scraper.py
    files_to_process = [
